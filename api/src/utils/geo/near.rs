...
pub async fn near(
    ...
    if tag == "random" {
        return random_features(lat, lon, mapbox_token).await;
    } else {
    ...
    if features.is_empty() && place_override {
        return random_features(lat, lon, mapbox_token).await;
    }
...
async fn random_features(lat: f64, lon: f64, mapbox_token: &String) -> Result<DCoord, StatusCode> {
